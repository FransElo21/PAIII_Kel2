...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Query langsung tanpa SP (pakai Eloquent/Builder, supaya search dan pagination mudah)
            ...
            $bookings = $query->orderByDesc('bookings.created_at')
                ->paginate(10)
                ->withQueryString();

            return view('admin.pemesanan', compact('bookings', 'search'));
        } catch (\Exception $e) {
    ...
    {
        try {
            $details = DB::select('CALL sp_get_booking_details(?)', [$id]);
